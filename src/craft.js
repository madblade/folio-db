    ...
    const articles = [
    ...
    
      {
        ...
        body: [
          ...
          {type: ParagraphType.Paragraph, content:
            '<b>Performance issues, III</b>: because of the generality of Threejs, it is necessary to use instancing for materials when rendering a scene from multiple cameras in a single frame (that’s the case for portals and water reflection). Without instancing materials and swapping between them after each render call, Threejs automatically rebuilds them (because it thinks the lighting has changed), greatly increasing the CPU load and inducing a performance drop, which is most noticeable in Firefox. The cause of this performance drop would have been difficult to determine without browser profiling tools. '
          },
          {type: ParagraphType.Paragraph, content:
            'Note (2021/04)—this isn’t actually an issue anymore because of the many changes that Threejs has been undergoing during this year. Browser implementations have also changed quite a lot, notably the support for the PointerLock API and Gamepad API. I might talk about all of this in the upcoming <i>Puddle Game</i> post-mortem. '
          },
          {type: ParagraphType.Paragraph, content:
