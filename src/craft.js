    ...
    const articles = [
      {
        ...
        body: [
          ...
          {type: ParagraphType.Paragraph, content:
            '<b>Texture bleeding</b>: using atlasing with texture filtering (linear mipmaps or anisotropic filtering) will produce bleeding artifacts on the edge of block tiles, because linear filters query neighbour tiles. Not entering into technical details, anisotropic filtering is performed by a WebGL extension called <emph>EXT_texture_filter_anisotropic</emph>, for which (sic.) “(…) implementations are permitted to ignore the minification or magnification filter and implement the highest quality of anisotropic filtering possible,” which means that most of the time, the minification filter is LINEAR_MIPMAP_LINEAR (where we want NEAREST). Without much control on how anisotropic filtering combines mipmaps in WebGL, there is little choice but to disable anisotropic filtering and only use nearest filters. That produces (ugly) grain at oblique viewing angles. '
          },
          {type: ParagraphType.Paragraph, content:
            '<b>Portals</b>: performance and flexibility. Using <emph>render target textures</emph> is a flexible solution for rendering nested portals (and allows one to further apply distortion effects if they so desire), at the cost of performance. The other well-documented technique, that uses <emph>stencil masks<emph>, offers performance at the cost of being somewhat less flexible and more programatically involved. Here I combine both: first, rendering a scene that contains only the portal frame to the stencil buffer, then rendering the “otherworldly” scene into an RTT using the stencil buffer to restrict the screen space needing rasterization. There is a neat performance gain over only using RTTs! In Threejs, however, this comes with a drawback: there is no default anti-aliasing for rendering to textures, hence the need for an additional software FXAA pass. '
          },
          {type: ParagraphType.Paragraph, content:
          ...
          {type: ParagraphType.Paragraph, content:
            '<b>Performance issues, II</b>: the clock provided by the JavaScript function <emph>performance.now()</emph> was accurate to 5µs, before Spectre and Meltdown. Since then, for security reasons, the resolution of timers has been decreased, and the support for SharedArrayBuffer has been dropped by many browsers, greatly limiting the advantages WebWorkers could have. '
          },
          {type: ParagraphType.Paragraph, content:
